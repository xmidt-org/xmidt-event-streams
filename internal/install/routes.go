...
type RoutesIn struct {
	...
	Routes           Routes
	Auth             *auth.Auth
	EventHandler     *event.RequestHandler
}
...
func provideCoreOption(server string, in RoutesIn) arrangehttp.Option[http.Server] {
	return arrangehttp.AsOption[http.Server](
		func(s *http.Server) {
			mux := chi.NewMux()
			if strings.ToLower(in.Routes.Events.Server) == server {
				mux.Method("POST", in.Routes.Events.Path, getHandlers(in.Auth.Middleware, in.EventHandler))
			}
			if server == "primary" {
